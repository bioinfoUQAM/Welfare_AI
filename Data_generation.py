...
class GeneratedData:
    ...
    def generate_samples(dataset, sheet_index, column_name):
        ...
        joint_column_modified = dataset[sheet_index][column_name].dropna()#remove NaN values
        for i, row in enumerate(joint_column_modified):
    ...
    @staticmethod
    def create_generated_cow_data(dataset, sheet_index):
        """
        ...
        """
        generated_data = []
        for i, column in enumerate(dataset[sheet_index]):
    ...
    def generate_monotone_sequences(data, order):
        ...
        for i, sequence in enumerate(sequences):
            if (i+1) < len(sequences):
                param = scipy.stats.norm.fit(data[sequences[i]+1: sequences[i+1]])# get the parameters
